...
class func{
	...
	function laporanStok(){
		$this->qry = mysql_query("SELECT * FROM barang_detail,barang WHERE (barang_detail.stok_awal!='0' OR barang_detail.stok_akhir!='0') AND barang.kode_barang=barang_detail.kode_barang ORDER BY barang_detail.kode_barang ASC");
		while($this->row = mysql_fetch_array($this->qry)){
	...
	function createLaporan($tgl1,$tgl2){
		$this->qry = mysql_query("SELECT max(tglmasuk) as tglmasuk, kode_barang,no_lpb, jumlah_masuk, stoks, keterangan FROM barang_masuk WHERE tglmasuk>='".mysql_escape_string($tgl1)."' AND tglmasuk<='".mysql_escape_string($tgl2)."' GROUP BY kode_barang");
		while($this->row = mysql_fetch_array($this->qry)){
			$this->databarang[]=array("kode_barang"=>$this->row['kode_barang'],"nobukti"=>$this->row['no_lpb'],"jumlah"=>$this->row['jumlah_masuk'],"tanggal"=>$this->row['tglmasuk'],"stoks"=>$this->row['stoks'],"keterangan"=>$this->row['keterangan']);
		}
		$this->qry = mysql_query("SELECT max(tanggal_keluar) as tanggal_keluar, kode_barang, no_do, jumlah_keluar, stoks, keterangan FROM barang_keluar WHERE tanggal_keluar>='".mysql_escape_string($tgl1)."' AND tanggal_keluar<='".mysql_escape_string($tgl2)."' GROUP BY kode_barang");				
		while($this->row = mysql_fetch_array($this->qry)){
			$this->databarang[]=array("kode_barang"=>$this->row['kode_barang'],"nobukti"=>$this->row['no_do'],"jumlah"=>$this->row['jumlah_keluar'],"tanggal"=>$this->row['tanggal_keluar'],"stoks"=>$this->row['stoks'],"keterangan"=>$this->row['keterangan']);
		}
		$this->qry = mysql_query("TRUNCATE TABLE laporan");
		$this->num = count($this->databarang);
		for($this->i=0;$this->i<$this->num;$this->i++){
			$this->qry = mysql_query("INSERT INTO laporan(kode_barang,nobukti,jumlah,tanggal,stoks,ket) VALUES('".$this->databarang[$this->i]['kode_barang']."','".$this->databarang[$this->i]['nobukti']."','".$this->databarang[$this->i]['jumlah']."','".$this->databarang[$this->i]['tanggal']."','".$this->databarang[$this->i]['stoks']."','".$this->databarang[$this->i]['keterangan']."')");
		}
		echo("<script>alert(\"Proses Selesai\");</script>");
	}
	function view_laporan(){
		$this->qry = mysql_query("SELECT max(tanggal) as tanggal,kode_barang,stoks FROM laporan WHERE kode_barang!='' GROUP BY kode_barang");
		while($this->row = mysql_fetch_array($this->qry)){
			$this->qry2 = mysql_query("SELECT * FROM barang WHERE kode_barang='".$this->row['kode_barang']."'");	
			$this->row2 = mysql_fetch_array($this->qry2);
			$this->databarang[] = array($this->row['kode_barang'],$this->row2['nama_barang'],$this->row['stoks']);
		}
		return $this->databarang;	
	}
}
